    ...
    if (props.difficulty === "hard") {
        return (
            ...
            >
                ...
                {props.hint ? <p className="mb-4 text-sm">{props.countries[props.currentIndex].code}</p> :
                    <><a href="#" onClick={props.showHint} className="my-4 text-xs hover:text-blue-600">
                        Get hint
                    </a><br/></>}
                <input onChange={e => props.setAnswer(e.target.value)} type="text" placeholder="Country name"
                       className="px-2 my-4 border rounded-xl"/><br/>
                {props.hintCapital ? <p className="mb-4 text-sm">{props.countries[props.currentIndex].capital.slice(0, 3)}</p> :
                <><a href="#" onClick={props.showHintCapital} className="my-4 text-xs hover:text-blue-600">
                    Get hint
                </a><br/></>}
